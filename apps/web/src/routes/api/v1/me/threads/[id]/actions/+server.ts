...
import { auditRecord, problem, readJsonValue, requireSession } from '../../../../../api-utils';
...
export const POST: RequestHandler = async (event) => {
  ...
  const thread = await db
    ...
    .where('thread.supporter_user_id', '=', session.userId)
    .where('payment.status', '=', 'succeeded')
    .executeTakeFirst();
  if (!thread) return problem(404, 'Thread not found');

  const actorKey = messageActorKey({ kind: 'user', userId: session.userId });
        ...
        await trx
          ...
          .values(
            auditRecord(
              ...
              {
                action: 'supporter.thread_blocked',
      ...
      await trx
        ...
        .values(
          auditRecord(
            ...
            { type: 'user', userId: session.userId },
            {
              action: 'supporter.thread_reported',
              resourceType: 'supporter_message_thread',
              resourceId: thread.id,
              projectId: thread.project_id,
